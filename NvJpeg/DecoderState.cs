...
namespace ManagedCuda.NvJpeg
{
    ...
    public class DecoderState : IDisposable
    {
        ...
        public void DecodeBatchedParseJpegTables(byte[] data)
        {
            res = NvJpegNativeMethods.nvjpegDecodeBatchedParseJpegTables(_nvJpeg.Handle, _state, data, data.Length);
            Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "nvjpegDecodeBatchedParseJpegTables", res));
            if (res != nvjpegStatus.Success)
                throw new NvJpegException(res);
        }

        public void DecodeBatchedParseJpegTables(IntPtr data, SizeT length)
        {
            res = NvJpegNativeMethods.nvjpegDecodeBatchedParseJpegTables(_nvJpeg.Handle, _state, data, length);
            Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "nvjpegDecodeBatchedParseJpegTables", res));
            if (res != nvjpegStatus.Success)
                throw new NvJpegException(res);
        }

        public void DecodeBatched(IntPtr[] data, SizeT[] lengths, nvjpegImage[] destinations, CUstream stream)
        {
