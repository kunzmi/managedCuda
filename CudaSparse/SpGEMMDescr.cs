...
namespace ManagedCuda.CudaSparse
{
	...
	/// </summary>
	public class SpGEMMDescr : IDisposable
	{
		private cusparseSpGEMMDescr descr;
		...
		protected virtual void Dispose(bool fDisposing)
		{
			...
			{
				//Ignore if failing
				res = CudaSparseNativeMethods.cusparseSpGEMM_destroyDescr(descr);
				Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cusparseSpGEMM_destroyDescr", res));
				disposed = true;
			}
			if (!fDisposing && !disposed)
		...
		/// </summary>
		public cusparseSpGEMMDescr Descr
		{
			get { return descr; }
		}
	}
