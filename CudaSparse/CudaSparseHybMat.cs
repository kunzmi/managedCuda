...
namespace ManagedCuda.CudaSparse
{
	...
	public class CudaSparseHybMat : IDisposable
	{
		...
		protected virtual void Dispose(bool fDisposing)
		{
			...
			{
				//Ignore if failing
				res = CudaSparseNativeMethods.cusparseDestroyHybMat(_hyb);
				Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cusparseDestroyHybMat", res));
				disposed = true;
			}
			if (!fDisposing && !disposed)
