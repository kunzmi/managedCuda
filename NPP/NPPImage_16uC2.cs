...
namespace ManagedCuda.NPP
{
	...
	/// 
	/// </summary>
	public class NPPImage_16uC2 : NPPImageBase
	{
		#region Constructors
		/// <summary>
		...
		public void MaxError(NPPImage_16uC2 src2, CudaDeviceVariable<double> pError, CudaDeviceVariable<byte> buffer)
		{
			...
			status = NPPNativeMethods.NPPi.MaximumError.nppiMaximumError_16u_C2R(_devPtrRoi, _pitch, src2.DevicePointerRoi, src2.Pitch, _sizeRoi, pError.DevicePointer, buffer.DevicePointer);
			Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "nppiFilterMedian_16u_C2R", status));
			NPPException.CheckNppStatus(status, this);
		}
		/// <summary>
		...
		{
			status = NPPNativeMethods.NPPi.ColorProcessing.nppiColorTwist32f_16u_C2R(_devPtr, _pitch, dest.DevicePointer, dest.Pitch, _sizeRoi, twistMatrix);
