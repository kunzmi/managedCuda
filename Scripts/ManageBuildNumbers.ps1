...
function Update-BuildVersionFromRevisionProjFile
{  
    [CmdletBinding()]  
    param()  
          
    $assemblyPattern = "[0-9]+(\.([0-9]+|\*)){1,3}"  
    $assemblyVersionPattern = '<AssemblyVersion>([0-9]+(\.([0-9]+|\*)){1,3})</AssemblyVersion>'  
      
    $foundFiles = get-childitem .\*GlobalAssemblyInfo.proj -recurse  
