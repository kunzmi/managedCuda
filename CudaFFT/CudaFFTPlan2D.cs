...
namespace ManagedCuda.CudaFFT
{
    ...
    /// and data type. This class is the same as <see cref="CudaFFTPlan1D"/> except that
  ...
  //      }
		...
		{
			SizeT size = new SizeT();
			res = CudaFFTNativeMethods.cufftGetSize2d(_handle, _nx, _ny, _type, ref size);
			Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cufftGetSize2d", res));
			if (res != cufftResult.Success)
				throw new CudaFFTException(res);
			return size;
		}
